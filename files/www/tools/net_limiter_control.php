...
// Fungsi untuk mendapatkan daftar perangkat yang terkoneksi beserta interface-nya
function getConnectedDevices() {
    $devices = array();
    // Menggunakan perintah ip neigh untuk mendapatkan informasi perangkat terkoneksi beserta interface
        ...
        foreach ($lines as $line) {
            if (!empty($line)) {
                ...
                if (count($parts) >= 5 && filter_var($parts[0], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
                    // Cari interface jika ada (biasanya di kolom ke-7 setelah 'dev')
                    $iface = '-';
                    $dev_index = array_search('dev', $parts);
...
function getActiveInterfaces() {
    ...
    // Pola interface yang umum untuk hotspot/tethering/usb/ethernet
    $patterns = [
        '/^wlan\\d+$/',    // WiFi
        '/^ap\\d*$/',      // Hotspot Android
        '/^usb\\d+$/',    // USB tethering
        '/^rndis\\d*$/',  // USB tethering Android
        '/^eth\\d+$/',    // Ethernet
        '/^ccmni-lan$/',   // MediaTek Tethering
        '/^cc-lan\\d*$/', // MediaTek Tethering
        '/^bt-pan$/',      // Bluetooth tethering
        '/^bnep\\d+$/',   // Bluetooth PAN
    ];

    foreach ($interfaceNames as $iface) {
        foreach ($patterns as $pattern) {
            if (preg_match($pattern, $iface)) {
                $interfaces[] = $iface;
                break;
            }
        }
                ...
                if (empty($devices)) {
                    ...
                    foreach ($devices as $device) {
                        ...
                        echo "<tr>
                                ...
                                <td>" . htmlspecialchars(isset($device['interface']) ? $device['interface'] : '-') . "</td>
                                <td>{$status_badges}</td>
                              </tr>";
